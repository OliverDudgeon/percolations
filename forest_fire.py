...
class ForestFire(BasePercolation):
    ...
    def enable(self, gui_manager):
        ...
        self.button_sim = UIButton(
            pygame.Rect(540, 700, 90, 50), "Simulate", gui_manager
        )
        self.menu_plot = UIDropDownMenu(
            ("Plot mean", "Plot raw data", "Plot order parameter"),
            "Plot mean",
            pygame.Rect(490, 750, 190, 20),
            gui_manager,
        )
        ...
        if event.user_type == pygame_gui.UI_BUTTON_PRESSED:
            ...
            if event.ui_element == self.button_sim:
                ...
                if self.menu_plot.selected_option == ("Plot mean"):
                    ...
                    self.plot_option_order = (
                        True  # Plot order parameter (also plots mean but much slower)
                    )
    ...
    def init_sim(self):
        ...
        if (
            self.plot_option_raw == True
        ):  # If plotting raw data the figure and axes already exist
            plt.xlabel("Initial tree fraction")
            plt.ylabel("Time until simulation complete")
        else:
            # Make array of tree coverage fractions used
            x_probs = np.linspace(0, 1, np.size(sim_times_final)).reshape(
    ...
    def sim_step(self):
        ...
        last_line = self.grid[
            self.grid_size
            ^ 2 - 2 * self.grid_size : self.grid_size
            ^ 2 - self.grid_size - 1
        ]
        ...
        if 2 in last_line:
            ...
            if self.plot_option_order == True:
                ...
                self.order_parameter_array[0, key_index] = number_of_fires / (
                    (self.grid_size - 2) ^ 2
                )
